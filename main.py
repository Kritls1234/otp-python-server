    ...
    async with semaphore:
        async with active_bot_request(bot_username):
            try:
                ...
                logger.info(
                    "get_otp done requestId=%s email=%s success=%s needButton=%s",
                    request_id,
                    mask_email(email),
                    result.get("success"),
    ...
    async with pending_lock:
        ...
        for key, pending in items:
            ...
            result = build_result_from_message(
                ...
                email=pending["email"],
                selected_button=pending["selected_button"],
                request_id=pending["request_id"],
                expect_buttons=pending["expect_buttons"],
                special_mode=pending["special_mode"]
            )
    ...
    if selected_lower:
        ...
        if is_household_choice(selected_lower):
            if (
                ...
                or "household" in text_lower
                or "travel" in text_lower
